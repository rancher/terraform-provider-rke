...
func flattenRKEClusterServicesScheduler(in rancher.SchedulerService) ([]interface{}, error) {
...
func expandRKEClusterServicesScheduler(p []interface{}) (rancher.SchedulerService, error) {
	...
	if v, ok := in["extra_args_array"].(string); ok && v != "" {
		array, err := jsonToMapStringSlice(v)
		if err != nil {
			return rancher.SchedulerService{}, err
		}
		obj.ExtraArgsArray = array
	}

	if v, ok := in["windows_extra_args_array"].(string); ok && v != "" {
		array, err := jsonToMapStringSlice(v)
		if err != nil {
			return rancher.SchedulerService{}, err
		}
