...
for i in build/bin/*; do
    ...
    BASE=build/archive
    DIR=${BASE}/terraform-provider-rke-${VERSION}
    ...
    cp $i ${DIR}/terraform-provider-rke${EXT}

    arch=$(echo $i | cut -f2 -d_)
    mkdir -p $DIST/${VERSION}/binaries/$arch
    mkdir -p $DIST/latest/binaries/$arch
    cp $i $DIST/${VERSION}/binaries/$arch/terraform-provider-rke${EXT}
    if [ -z "${EXT}" ]; then
        gzip -c $i > $DIST/${VERSION}/binaries/$arch/terraform-provider-rke.gz
        xz -c $i > $DIST/${VERSION}/binaries/$arch/terraform-provider-rke.xz
    fi
